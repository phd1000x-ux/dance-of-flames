...
export class BuildingSystem {
  ...
  applyFireDamage(origin: Vector3, dir: Vector3, range: number, halfAngle: number, dps: number, dt: number): void {
    ...
    for (const b of this.buildings) {
      ...
      if (cosA > cosCone || v.length() < radius + 6) {
        ...
        b.burnAccum += dt;
        // heat up visual
        const heat = 1 - b.hp / b.maxHp;
        b.material.emissiveColor = new Color3(heat * 0.35, heat * 0.08, 0);
        if (b.hp < b.maxHp * 0.55 && !b.firePs) {
          this.attachFire(b);
        }
  ...
  private collapse(b: BuildingEntity): void {
    ...
    this.effects.dust(b.pos.subtract(new Vector3(0, b.size.h / 2, 0)), Math.max(1, b.size.w / 8));
    this.effects.explosion(b.pos, Math.max(1, b.size.w / 7));
    this.bus.emit("sfx", { name: "buildingCollapse" });
    this.onShakeRequest?.(b.pos, Math.min(1.0, b.size.w / 12));
    if (b.firePs) {
